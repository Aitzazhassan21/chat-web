...
const Chat = ({ chat: chatData, onExit }) => {
  ...
  const handleUsernameClick = () => {
    // Check if the screen width is larger than 1024px (for large screens)
    if (window.innerWidth > 1024) {
      Swal.fire({
        icon: "info",
        title: "You Are Already",
        text: "Show on the screen Detail page",
        confirmButtonText: "OK",
      });
    } else {
      setShowDetail(true);
      // Handle behavior for smaller screens (if needed)
      console.log("Small screen click");
    }
  };
  ...
  const handleImg = (e) => {
    if (e.target.files[0]) {
        setDocFile({ file: null, url: "" }); // Reset document file
      setImg({
  ...
  const handleDoc = (e) => {
    if (e.target.files[0]) {
      setImg({ file: null, url: "" }); // Reset image file
      setDocFile({
        file: e.target.files[0],
        url: URL.createObjectURL(e.target.files[0]),
      });
  ...
  const handleSend = async () => {
    if ((img.file && docFile.file) ||(text === "" && !img.file && !docFile.file)) return;
  ...
  return (
    <div className="chat">
      {showDetail ? (
        <Detail onExit={handleExitDetail} user={chat.user} />
      ) : (
        <>
          <div className="top">
            ...
            >
              ...
              <div className="texts">
                ...
                {onlineStatus ? (
                  ...
                  <small style={{ color: "skyblue", fontSize: "12px" }}>
                    {getLastSeenText()}
                  </small>
                )}
              </div>
          ...
          <div className="bottom">
            ...
            <IoSendSharp
              ...
              onClick={handleSend}
              disabled={
                isCurrentUserBlocked || isReceiverBlocked ? true : undefined
              }
            />
          </div>
        </>
